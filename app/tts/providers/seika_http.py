...
def _build_save2_url(base: str, cid: int, text: str, sr: int, speed: float, pitch: float) -> str:
    ...
    return f"{base}/SAVE2/{cid}/{txt}?sr={sr}&speed={speed}&pitch={pitch}&format=wav&play=0"

class SeikaHTTP:
    def __init__(self, base: str | None = None):
        self.base = (base or SEIKA_BASE).rstrip("/")

    def synth_wav(self, cid: str, text: str, *, speed=1.0, pitch=0.0, samplerate=48000) -> bytes:
        if not text:
            return b""
        url = _build_save2_url(self.base, cid, text, samplerate, speed, pitch)
